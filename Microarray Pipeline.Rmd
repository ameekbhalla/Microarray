...
```{r expression_plots_w/_adj_w/o_FC}
...
The expression levels of most genes overlapped a lot b/n the control and test samples since they were selected based on only their p-values; to reduce the overlap, a logFC cutoff of >=1.2 was added to the selection criteria---the same principle which underlies a volcano plot.

```{r venn_overlap_w/_adj_w/_FC}

# Genes that are DE across two generations with logFC >= 1.2; p-values corrected by the empirical null method
geno_1v2_2 <- geno_1 %>% filter(pval <= 0.05 & abs(logFC) >= 1.2) %>% 
  inner_join( geno_2 %>% filter(pval <= 0.05 & abs(logFC) >= 1.2))
# 166 DE genes in first generation; 
# no genes found that are DE in second or third generation, hence no trangen gene overlap

geno_2v3_2 <- geno_2 %>% filter(pval <= 0.05 & abs(logFC) >= 1.2) %>% 
  inner_join( geno_3 %>% filter(pval <= 0.05 & abs(logFC) >= 1.2))
# no genes found that are DE in second or third generation, hence no trangen gene overlap

geno_1v3_2 <- geno_1 %>% filter(pval <= 0.05 & abs(logFC) >= 1.2) %>% 
  inner_join( geno_3 %>% filter(pval <= 0.05 & abs(logFC) >= 1.2))
# 166 DE genes in first generation; 
# no genes found that are DE in second or third generation, hence no trangen gene overlap
...
Conclusion:
1.   Corrected, w/o FC threshold, w/  p value adj ---    DE genes found in 2nd & 3rd generation --- use latter 2 only
2.   Corrected, w/o FC threshold, w/o  p value adj ---    DE genes found in 2nd & 3rd generation --- 
3.   Corrected, w/  FC threshold, w/  p value adj --- no DE genes found in 2nd & 3rd generation --- ignore
4.   Corrected, w/  FC threshold, w/o  p value adj --- no DE genes found in 2nd & 3rd generation --- not done
5. Uncorrected, w/  FC threshold, w/  p value adj --- no DE genes found in 2nd & 3rd generation --- ignore 
6. Uncorrected, w/  FC threshold, w/o p value adj --- no DE genes found in 2nd & 3rd generation --- more 1st DE genes than in #1    --- *use for displaying expression levels and building volcano plots*
7. Uncorrected, w/o FC threshold, w/o p value adj --- no DE genes found in 2nd & 3rd generation --- most 1st DE genes than in #1/#4 --- *use with #1 to give to Abhay and for geo* 
8. Uncorrected, w/o FC threshold, w/o p value adj --- no DE genes found in 2nd & 3rd generation --- not done

To do:
[] - Repeat with other sex
[] - Prepare Final List with #1 and #5
