...
fit2_model_1 <- contrasts.fit(fit2_model_1, cm_1_genotype)
...
top_1_geno <- topTable(fit2_model_1,
                  # coef=c(1:2),
                  adjust="BH", number = nrow(fit2_model_1), sort.by = "none")

hist(top_1_geno$P.Value)
...
fdr_1 <- (top_1$t - median(top_1$t)) %>% fdrtool(statistic= "normal", plot = F)

qplot(fdr_1$pval, geom="histogram", binwidth = 0.05, boundary = 0.05, color = I("black"), fill = I(NA)) + ggtitle("p-value histogram of group-means interaction model, corrected (via t-stats)") +
  theme_classic() +
