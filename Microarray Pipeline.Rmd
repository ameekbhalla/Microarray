...
eset_list <- list()
eset2_list <- list()
# to start the for loop at zero, replace seq_along(1:15) with c(0:15) and [[i]] with [[i+1]]
...
annotation_for_heatmap <- 
  data.frame(
    sex = pData(eset)$sex,
    # # batch = pData(eset)$replicate,
    generation = pData(eset)$generation,
...
annotation_for_heatmap <- annotation_for_heatmap %>%
  mutate(
    ...
    genotype = case_when( # categorize genotypes into test or control lineages
      genotype == "rpr/cyo; dilp3/ser" ~ "t",
      genotype == "rpr/+; dilp3/+" ~ "2_t",
      genotype == "cyo/+; dilp3/+" ~ "3_t",
      genotype == "rpr/+; ser/+" ~ "4_t",
      genotype == "cyo/+; ser/+" ~ "1_t",
      genotype == "test" ~ "t",
      TRUE ~ "c") %>% as_factor(),
    # Genotype = case_when( # convert all non-transgenic lineages into w1118 genotype
    #   genotype == "rpr/cyo; dilp3/ser" ~ "rpr/cyo; dilp3/ser",
    ...
    #   genotype == "cyo/+; ser/+" ~ "cyo/+; ser/+",
    #   TRUE    ~ "w1118") %>% as_factor(),
    sex = case_when( # convert male to m and female to f
      sex == "female" ~ "f",
      sex == "male" ~ "m") %>% as_factor()
    )

# annotation_for_heatmap$genotype <- NULL
annotation_for_heatmap$serial <- 1:66


annotation_for_heatmap$group <- with(annotation_for_heatmap, 
                            paste(
                              serial,
                              generation,
                                  # replicate,
...
row.names(annotation_for_heatmap) <- annotation_for_heatmap$group

exp_eset <- Biobase::exprs(eset)
dists <- as.matrix(dist(t(exp_eset), method = "manhattan"))

rownames(dists) <- annotation_for_heatmap$group
hmcol <- rev(colorRampPalette(RColorBrewer::brewer.pal(9, "YlOrRd"))(255))
colnames(dists) <- NULL
diag(dists) <- NA
...
pheatmap::pheatmap(dists, col = (hmcol), 
         ...
         legend_breaks = c(min(dists, na.rm = TRUE), 
                         max(dists, na.rm = TRUE)), 
         legend_labels = (c("small distance", "large distance")),
         main = "Clustering heatmap (both sexes)"
         )
...
my_heatmap <- pheatmap::pheatmap(dists, col = (hmcol), 
         annotation_row = annotation_for_heatmap,
         annotation_colors = ann_colors,
         legend = TRUE, 
         treeheight_row = 0,
...
my_heatmap$tree_row %>%
  as.dendrogram() %>%
  plot(horiz = FALSE, main = "Clustering heatmap (both sexes)")

#####
#using coolmap, clustering can be done based on DE expression of a subset of genes rather than average expression as described in the URL below
...
exprs(eset_f2_f) %>% coolmap(show.dendrogram = "both")

exprs(eset_f2_f) %>% coolmap(show.dendrogram = "column")

exprs(eset_f2_f) %>% coolmap(show.dendrogram = "none")
...
mini <-  which(
  ...
  pData(eset)$genotype   != "rpr/+; dilp3/+" #& #commented out to include both sexes
  # pData(eset)$genotype   != "rpr/cyo; dilp3/ser" &
  # pData(eset)$sex        != "female" &
  # pData(eset)$sex        != "male" #commented out to include both sexes
                )
...
corfit_1 <- duplicateCorrelation(eset_mini, design_1, block =  pData(eset_mini)$group2) #group2 from correlations.R where blocking is done on both replicate and sex
corfit_1$consensus
...
fit_model_1 <- lmFit(eset_mini, design_1, 
                     block =  pData(eset_mini)$group2, #group2 from correlations.R where blocking is done on both replicate and sex
...
keep <- fit_model_1$Amean > 8 #`Amean` stores the average log expression values; >8 cutoff gives most normal-looking expression densities
...
geno_0 <- bind_cols(top_1_geno_0, fdr_1_0) %>% rownames_to_column() %>% mutate(pval = P.Value, qval = adj.P.Val)
geno_1 <- bind_cols(top_1_geno_1, fdr_1_1) %>% rownames_to_column() %>% mutate(pval = P.Value, qval = adj.P.Val)
