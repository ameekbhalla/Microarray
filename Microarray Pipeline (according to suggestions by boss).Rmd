...
rpr.cyo_dilp3.ser_f %>% mutate(rpr.cyo_dilp3.ser_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0) %>% as.logical()) %>% select(-c(5:7)) %>%
  full_join(
cyo_ser_f %>% mutate(cyo_ser_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0) %>% as.logical()) %>% select(c(1,8)), by = "probe_id") %>%
  full_join(
# rpr_dilp3_f %>% mutate(rpr_dilp3_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0) %>% as.logical()) %>% select(c(1,8)), by = "probe_id") %>%
#   full_join(
# cyo_dilp3_f %>% mutate(cyo_dilp3_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0) %>% as.logical()) %>% select(c(1,8)), by = "probe_id") %>%
  # full_join(
# rpr_ser_f %>% mutate(rpr_ser_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0) %>% as.logical()) %>% select(c(1,8)), by = "probe_id") %>%
  # full_join(
f2_f %>% mutate(f2_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0) %>% as.logical()) %>% select(c(1,8)), by = "probe_id") %>% 
  full_join(
f3_f %>% mutate(f3_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0) %>% as.logical()) %>% select(c(1,8)), by = "probe_id") %>% 
  ...
  filter(map_dbl(.x = test$groups, length) >= 2) %>% #keep only rows which occur in two or more sets
  ggplot(aes(x = groups)) +
  geom_bar() +
  scale_x_upset()
```
