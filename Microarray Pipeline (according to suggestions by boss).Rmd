...
keep <- which(pData(raw_data)$generation != "F-1" & pData(raw_data)$generation != "F0 + F1" & pData(raw_data)$generation != "F2" & pData(raw_data)$sex != "female")
...
# # effect of gcrma
# plotDensities(exprs(eset) %>% log2(), 
#               group = pData(raw_data)$generation, legend = "topright",
#               main = "Variation in expression according to generation (raw data)")
```
...
dataGG <- data.frame(PC1 = PCA_raw$x[,1], PC2 = PCA_raw$x[,2],
                    # Phenotype = pData(eset)$Factor.Value.phenotype.,
                    # Individual = pData(eset)$Characteristics.individual.
                    sex = pData(eset)$sex
                    )
...
ggplot(dataGG, aes(PC1, PC2)) +
  ...
  ggtitle("Most of the variation is due to generation, after removing sex") +
  xlab(paste0("PC1, VarExp: ", percentVar[1], "%")) +
  ylab(paste0("PC2, VarExp: ", percentVar[2], "%")) +
  theme(plot.title = element_text(hjust = 0.5))+
  coord_fixed(ratio = sd_ratio) +
...
exp_raw <- log2(Biobase::exprs(eset_m_wt))
PCA_raw <- prcomp( #Performs PCA on given matrix and returns results as an object of class prcomp
  t(exp_raw), #transpose
  scale. = FALSE
  )
...
dataGG <- data.frame(PC1 = PCA_raw$x[,1], PC2 = PCA_raw$x[,2],
                    generation = pData(eset_m_wt)$generation,
                    genotype = pData(eset_m_wt)$genotype,
                    replicate = pData(eset_m_wt)$replicate,
                    Batch = pData(eset_m_wt)$day
                    )
...
ggplot(dataGG, aes(PC1, PC2)) +
      geom_point(aes(shape = Lineage, colour = Lineage)) +
  ggtitle("Most of the variation is due to generation, after removing sex") +
  xlab(paste0("PC1, VarExp: ", percentVar[1], "%")) +
  ylab(paste0("PC2, VarExp: ", percentVar[2], "%")) +
  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
palmieri_eset <- rma(raw_data, target = "core", normalize = FALSE)
...
row.names(annotation_for_heatmap) <- pData(eset_m)$label
```

```{r}
exp_palmieri <- Biobase::exprs(eset_m)
dists <- as.matrix(dist(t(exp_palmieri), method = "manhattan"))

rownames(dists) <- pData(eset_m)$label
...
geno_0 <- cbind(top_1_geno_0, fdr_1_0) %>% rownames_to_column() %>% 
  select("rowname", "logFC", "pval", "qval")
geno_1 <- cbind(top_1_geno_1, fdr_1_1) %>% rownames_to_column() %>% 
  select("rowname", "logFC", "pval", "qval") %>% left_join(anno_eset, by = c("rowname" = "PROBEID"))
geno_2 <- cbind(top_1_geno_2, fdr_1_2) %>% rownames_to_column() %>% 
  select("rowname", "logFC", "pval", "qval")
geno_3 <- cbind(top_1_geno_3, fdr_1_3) %>% rownames_to_column() %>% 
  select("rowname", "logFC", "pval", "qval")


library(XLConnect)
by_generation <- loadWorkbook("by_generation.xlsx", create = TRUE)
createSheet(by_generation, "F1")
writeWorksheet(by_generation, geno_1, "F1")
saveWorkbook(by_generation, "by_generation.xlsx")
