...
```{r upset}
rpr.cyo_dilp3.ser_f %>% mutate(rpr.cyo_dilp3.ser_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0)) %>% select(-c(5:7)) %>%
  full_join(
cyo_ser_f %>% mutate(cyo_ser_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0)) %>% select(c(1,8)), by = "probe_id") %>%
  full_join(
rpr_dilp3_f %>% mutate(rpr_dilp3_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0)) %>% select(c(1,8)), by = "probe_id") %>%
  full_join(
# cyo_dilp3_f %>% mutate(cyo_dilp3_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0)) %>% select(c(1,8)), by = "probe_id") %>%
  # full_join(
# rpr_ser_f %>% mutate(rpr_ser_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0)) %>% select(c(1,8)), by = "probe_id") %>%
  # full_join(
f2_f %>% mutate(f2_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0)) %>% select(c(1,8)), by = "probe_id") %>% 
  full_join(
f3_f %>% mutate(f3_f = case_when(qval <= 0.05 ~ 1, TRUE ~ 0)) %>% select(c(1,8)), by = "probe_id") -> test

test %>% 
  # drop_na() %>%
  # mutate(sum = rowSums(.[5:9])) %>% filter(sum != 0) %>% select(-c(1:4, 10)) %>%
  upset(
    # order.by = "freq"
    )
```
...
join_degs <- function(df1, df2){
  df1 %>% get() %>% filter(qval <= 0.05) %>% select(probe_id, logFC) %>% 
    inner_join(
  df2 %>% get() %>% filter(qval <= 0.05) %>% select(probe_id, logFC), by = "probe_id", suffix = c("1", "2")
    ) %>% mutate(group = expr_label(df2))
  }

#run the common degs() function on all the dfs in the environment
map2_dfr(.x = "rpr.cyo_dilp3.ser_m",
         .y = df_list,
     .f = join_degs
...
=======
```{r correlations}
#correlation between the logFC of DE genes

test1 <- rpr.cyo_dilp3.ser_m %>% filter(qval <= 0.05) %>% pluck("qval")
test2 <- rpr.cyo_dilp3.ser_f %>% filter(qval <= 0.05) %>% pluck("qval")  

library(MASS)

boxcox(lm(test1 ~ test2))
#this suggests that the seventh root or eight root transformation might be good for normalizing the data

#the data is still not linear
ggplot(data = test3, aes((qval.1)^(1/8), (qval.2)^(1/8))) + geom_point() 

>>>>>>> cc5968c184747fdf6fd4e1f0e6c3b07619986ed6
```
