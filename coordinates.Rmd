...
sheet_1$genes <- vector(mode = "list", length = nrow(sheet_1))

for (i in seq(1:8428)) {
sheet_1$genes[[i]]  <- sheet_2 %>% slice(which(sheet_1$chromosome[i] == sheet_2$chromosome_2 & 
                                               sheet_1$end[i] >= sheet_2$start & sheet_2$start >= sheet_1$start[i])) %>%
  pluck("gene") %>% as.list()
  }

sheet_2 %>% slice(which(sheet_1$chromosome[1] == sheet_2$chromosome_2 & 
                                               sheet_1$end[1] >= sheet_2$start & sheet_2$start >= sheet_1$start[1])) %>%
  pluck("gene") %>% as.list()
```
