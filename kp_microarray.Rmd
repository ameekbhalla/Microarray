...
kp_samples <- kp_samples |> dplyr::select(generation, media, replicate, temperature, genotype) |> 
  mutate(
  generation = generation |> as.factor(),
  media = media |> factor(levels = c("LS", "HS")),
  replicate = replicate |> as.factor(),
  temperature = temperature |> as.factor(),
  genotype = case_when( # categorize genotypes into test or control lineages
    ...
    # `sample code` == "15T" ~ "15T",
    # `sample code` == "16T" ~ "16T",
    # `sample code` == "T1F2" ~ "T1F2",
    # `sample code` == "T2F2" ~ "T2F2",
    # `sample code` == "T3F2" ~ "T3F2",
    # `sample code` == "T4F2" ~ "T4F2",
    # `sample code` == "T5F2" ~ "T5F2",
    # TRUE ~ "other"),
  label = paste(generation, media, genotype, temperature, sep = "_") |> as.factor()
  )

raw <- ReadAffy(celfile.path = "./kp CEL")

which(kp_samples$`sample code` == "9W")
which(kp_samples$`sample code` == "T2F1")
# which(kp_samples$`sample code` == "13T")
# which(kp_samples$`sample code` == "T4F2")

kp_samples <- kp_samples[-c(9, 13), ]
raw <- raw[ , -c(9, 13)]

phenoData(raw) <- AnnotatedDataFrame(kp_samples)

sampleNames(raw) <- paste(kp_samples$generation, 
                             kp_samples$genotype, 
                             kp_samples$temperature, 
                             kp_samples$replicate,
                             sep = "_")

all(colnames(exprs(raw)) == rownames(pData(raw)))

all(colnames(exprs(raw)) == sampleNames(raw))

all(rownames(pData(raw)) == sampleNames(raw))

all(rownames(exprs(raw)) %>% head() == rownames(fData(raw)) %>% head())
...
raw_expression <- exprs(raw) %>% #plotDensities() accepts only matrix/ExpressionSet objects
                  log2() #the data is log2 scaled to linearize it across different orders of magnitude 

plotDensities(raw_expression, group = pData(raw)$genotype, legend = "topright",
              main = "Variation in expression according to genotype (raw)")

norm_expression <- exprs(eset) %>% #plotDensities() accepts only matrix/ExpressionSet objects
                  log2() #the data is log2 scaled to linearize it across different orders of 

plotDensities(norm_expression, group = pData(eset)$genotype, legend = "topright",
              main = "Variation in expression according to genotype (eset)")
```

```{r qc_metrics eval = FALSE}
library("arrayQualityMetrics")

arrayQualityMetrics(expressionset = raw, # this can be an AffyBacth object
...
# samples T2F1, 9W are problematic
```
...
design_1 <- model.matrix(~ 0 + label, data = pData(eset))

colnames(design_1) <- levels(pData(eset)$label)
colnames(design_1)
...
# corfit_1 <- duplicateCorrelation(eset, design_1, block =  pData(eset)$replicate)
# corfit_1$consensus
...
cm <- makeContrasts(
  f0_t25_v_c25 = (F0_LS_test_25d - F0_LS_control_25d),
  f0_t16_v_c16 = (F0_LS_test_16d - F0_LS_control_16d),
  f0_t25_v_t16 = (F0_LS_test_25d - F0_LS_test_16d),
  f0_c25_v_c16 = (F0_LS_control_25d - F0_LS_control_16d),
  f1_t25_v_c25 = (F1_HS_test_25d - F1_HS_control_25d),
  f2_t25_v_c25 = (F2_HS_test_25d - F2_HS_control_25d),
  levels = design_1)

cm

#fit the contrast matrix
fit2_model_1 <- contrasts.fit(fit_model_1, cm)
