...
kp_results_list[[1]] <- F0_t25_v_c25 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results_list[[2]] <- F0_t16_v_c16 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results_list[[3]] <- F0_t25_v_t16 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results_list[[4]] <- F0_c25_v_c16 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results_list[[5]] <- F0_t25_v_c16 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results_list[[6]] <- F0_t16_v_c25 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results_list[[7]] <- F1_t25_v_c25 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results_list[[8]] <- F2_t25_v_c25 |> rownames_to_column() %>% left_join(anno_eset, by = c("rowname" = "PROBEID")) %>% dplyr::rename(probe_id = rowname, symbol = SYMBOL, gene_name = GENENAME, entrez_id = ENTREZID)

kp_results <- write_xlsx(kp_results_list)
```