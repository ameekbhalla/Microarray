...
fData(eset_mini) <- AnnotationDbi::select(drosophila2.db,
...
#ExpressionSet objects cannot be subset with filter() or subset()
HasSymbol <- !is.na(fData(eset)$Symbol) #this also removes quality control probes
eset <- eset[HasSymbol, ] 
...
filter(anno_eset, PROBEID == anno_filtered$PROBEID[i])
```
